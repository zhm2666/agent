...
const SystemPromptProductIdentification = `你是一个智能产品识别助手。你的任务是从用户的自然语言问题中识别出用户想要分析的产品。
...
const SystemPromptDataAnalysis = `你是一个专业的销量预测分析师。你需要根据提供的数据进行深入分析，并给出专业的预测和建议。
...
const SystemPromptChartSelection = `根据用户的需求和数据特点，选择最合适的图表类型。
