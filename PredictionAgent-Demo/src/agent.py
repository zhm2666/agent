...
from .utils import Config, load_config, ChartHelper
...
class PredictionAgent:
    ...
    def __init__(self, config: Optional[Config] = None):
        ...
        # 初始化图表助手
        self.chart_helper = ChartHelper(self.config.chart_output_dir)
    ...
    def _initialize_nodes(self):
        ...
        self.chart_node = ChartNode(self.llm_client, self.chart_helper)
    ...
    def analyze(
        ...
        Args:
            ...
            product_code: 直接指定产品代码（跳过产品识别步骤）

        Returns:
            包含分析结果的字典
        """
        print(f"\n{'='*60}")
        print(f"开始预测分析: {query}")
        print(f"{'='*60}")
    ...
    def _step_product_identification(self, query: str, product_code: Optional[str] = None):
        ...
        if product_code:
            ...
            if product:
                ...
                self.state.prediction_state.product_identification.reasoning = "直接指定产品代码"
                print(f"已识别产品: {product.product_name} ({product.product_code})")
                return
            else:
                print(f"未找到产品: {product_code}")
        ...
        if id_state.identified:
            ...
            print(f"置信度: {id_state.confidence:.2%}")
        else:
            print("未能识别产品")
    ...
    def _step_data_fetch(self, use_mock_data: bool = False):
        ...
        if fetch_state.fetched:
            print(f"数据获取成功:")
            print(f"  - 历史数据: {len(fetch_state.historical_data)} 条")
            print(f"  - 未来预测: {len(fetch_state.future_predictions)} 条")
            if fetch_state.statistics:
                avg = fetch_state.statistics.get("avg_daily_sales", 0)
                trend = fetch_state.statistics.get("trend_direction", "unknown")
                print(f"  - 日均销量: {avg:.2f}")
                print(f"  - 趋势: {trend}")
        else:
            print(f"数据获取失败: {fetch_state.error_message}")
    ...
    def _step_chart_generation(self):
        ...
        if chart_state.generated:
            print(f"图表生成成功: {chart_state.chart_url}")
        else:
            print(f"图表生成失败")
